...
public class NewMotionBlur : ScriptableRendererFeature
{
    [Serializable, VolumeComponentMenu("Ding Post-processing/12.6 MotionBlur")]
    public class CustomVolumeComponent : DingVolumeComponentBase 
    {
        public ClampedFloatParameter BlurAmount = new ClampedFloatParameter(0.5f, 0, 0.9f);
    ...
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using(new ProfilingScope(cmd, m_ProfilingSampler)){
                ...
                material.SetFloat("_BlurAmount", 1.0f - volume.BlurAmount.value);
                //将当前帧的colorRT用着色器（shader in material）渲染后输出到之前创建的贴图（辅助RT）上
                Blit(cmd, source, tempTexture.Identifier(), material, 0);
                Blit(cmd, source, tempTexture.Identifier(), material, 1);
